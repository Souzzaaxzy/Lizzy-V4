...
### Modo REMOTO EXCLUSIVO via `akinator-client` (set/2026) ✅
Pedido do dono: *"faça a bot usar o remoto exclusivamente o akinator que você
instalou agora"*.

- `AKINATOR_MODE=remoto` e' o **PADRAO**. O `!akinator` usa **so'** o
  Akinator.com (`akinator-client@^1.3.0`).
- **EXCLUSIVO**: `REMOTO_EXCLUSIVO = true`. Se o Akinator nao responder, o
  comando **AVISA** (`remoto_indisponivel` + `mensagemRemotoIndisponivel()`) e
  **nao joga** -- nada de trocar para o engine proprio escondido.
- `AKINATOR_MODE=local` = engine proprio (opcional).
- `AKINATOR_REMOTO_FALLBACK=local` = reabilita a queda automatica (opt-in).
- `!akinator status` mostra o motor e, se o remoto caiu, o motivo.

**IMPORTANTE (medido, nao suposto)**: neste VPS o Akinator.com responde **403
"Just a moment..."** (challenge do Cloudflare) e o pacote falha com *"Failed to
extract session/signature from HTML response"*. Ou seja: **com o modo exclusivo,
o `!akinator` NAO funciona neste servidor** -- ele avisa o motivo em vez de
jogar. Para funcionar, precisa de proxy residencial (`AKINATOR_PROXY`) ou
ScraperAPI. Foi implementado como o dono pediu, e o aviso e' honesto na mensagem.

**BUG DE BORDA corrigido no caminho** (achado por teste *flaky*): o cache
comparava `Date.now() - mtime < ttl` e exigia `idade >= 0`. Quando o `mtime`
ficava alguns ms ADIANTADO do relogio (resolucao do FS), um cache recem-escrito
parecia **vencido** e era baixado de novo. Agora ha **tolerancia de skew**
