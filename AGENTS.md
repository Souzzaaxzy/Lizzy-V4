...
## CORREÇÃO REGRESSÃO !play (set/2026) — 'Requested format is not available' ✅
- **Causa raiz**: forçar `--extractor-args youtube:player_client=web_safari|mweb|web` (commit `bedccee`, em busca de latência) faz o YouTube devolver **só storyboards** em vários vídeos (zero formatos de áudio)→ `Requested format is not available`. O client `default` do yt-dlp (extração nativa, **sem** `--extractor-args`) é o único que expõe formatos reais nesses casos.

- **Correção** em `youtube.js` (`defaultClients`): `['default', 'web_safari', 'mweb', 'web', 'android_vr', 'android']`; client `default` usa `passArgs` puro (sem `--extractor-args`; clients forçados só como fallback. **Rate-limit transitório no default**: reexecuta o `default` uma vez (backoff 4s, `repeatDefaultDone`); 2ª falha → `skipForcedClients` pula os clients forçados (que só devolveriam storyboards), falhando rápido com msg de bloqueio (não "Formato indisponível").
- **Cadeia mp3 dinâmica** (sem fixar IDs): `bestaudio[ext=m4a][abr<=128]/bestaudio[ext=m4a]/bestaudio[abr<=128]/bestaudio[ext=mp4]/bestaudio` — yt-dlp escolhe o que o vídeo expõe; `bestaudio` universal no fim.

- **Diagnóstico**: se todos falharem por `Requested format is not available` sem bloqueio/auth → log `[YT-DLP] Nenhum formato compatível encontrado` e msg ao usuário sobre contexto/cookies/IP (sem stack trace, sem expor cookies/credenciais).
- **Cookies preservados**: `YOUTUBE_COOKIES_FILE`/`YTDLP_COOKIES_FILE`/`data/youtube/cookies.txt` intactos; fallback controlado sem cookies numa 2ª passada só quando bloqueio/auth (mantido do commit bedccee.
- **`[PLAY][PERF]`** logs: `yt-dlp download+conversão` e `leitura do arquivo` (medem o gargalo por execução; úteis em produção, sem expor nada sensível).
- **Medido no sandbox (IP datacenter, sem cookies**: mp3 128k `!play` ~6.5-7.7s (1ª vez), cache hit 0.0s; mp4 360p ~3.4-4.3s (média 3.8s). Vídeos com rate limit ambiental do sandbox → msg "YouTube bloqueou temporariamente" (honesta, não "Formato indisponível". Em produção com cookies o `default` autenticado baixa sem esse bloqueio.
