...
## CORREÇÃO YouTube (ago/2026) — getPlayer multi-client ✅
- `dados/src/funcs/downloads/youtube.js`: `getPlayer(videoId)` agora faz **fallback ordenado entre 4 clients Innertube** (`PLAYER_CLIENTS`): `ANDROID@20.10.38`, `ANDROID@20.19.35`, `IOS@20.10.4`, `ANDROID_VR@1.61.48` — cada um com UA próprio. Aceita só `playabilityStatus OK` + formatos com URL direta; `LOGIN_REQUIRED` (anti-bot intermitente) ganha 1 retry por client; erro final controlado com diagnóstico por client via `console.warn`.
- Fatos empíricos (sandbox, ago/2026): ANDROID < 20.x → HTTP 400 (versão morta); WEB/MWEB/TVHTML5/EMBEDDED → UNPLAYABLE/ERROR sem poToken; respostas são **intermitentes** (mesmo client+vídeo alterna OK/LOGIN_REQUIRED); URLs de stream desses 4 clients vêm sem cifra "n"; parte do conteúdo licenciado (ex.: Blinding Lights) é bloqueada por IP de datacenter (até a watch page falha) — em IP residencial/VPS funciona.
- Busca continua `yt-search` (inalterada). Pipeline FFmpeg/download inalterada. Bug secundário conhecido (não corrigido, fora de escopo): `index.js` ~19172 usa `videoInfo.data.id` (inexistente — search retorna `data.videoId`); thumbnail cai no fallback `data.thumbnail`, fluxo não quebra. `playvid` caption usa `data.author.name` mas `author` é string → imprime "undefined" (pré-existente).
