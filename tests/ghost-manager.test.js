...
await test('handler: !addghostcmd gera key, vincula e envia tutorial + arquivo', async () => {
  ...
  includes(r.texto, 'antifantasma.cjs', 'explica onde colocar o arquivo');
  includes(r.texto, "require('./antifantasma.cjs')", 'explica o import');
  ...
  includes(r.texto, 'antiFantasma.iniciar', 'explica que a CASE liga a proteção contínua');
  includes(r.texto, 'Reiniciar', 'manda reiniciar o bot');
  ...
  includes(fimTutorial, "require('./antifantasma.cjs')", 'bloco final traz o require');
  ...
  includes(fimTutorial, 'iniciar(', 'bloco final liga a proteção contínua');
  notIncludes(fimTutorial, 'groupData.antiinvi =', 'não escreve mais no estado antigo do grupo');

  ok(Boolean(r.doc), 'enviou o arquivo');
  ok(r.doc.content.fileName === 'antifantasma.cjs', `arquivo correto (${r.doc.content.fileName})`);
});
