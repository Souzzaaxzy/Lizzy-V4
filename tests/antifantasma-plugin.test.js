...
fs.copyFileSync(path.join(PROJECT, 'dados', 'src', 'antifantasma-cliente', 'antifantasma.cjs'), ADAPTADOR_CJS);
...
await test('adaptador: a lógica NÃO está no arquivo entregue', () => {
  const fonte = fs.readFileSync(path.join(PROJECT, 'dados', 'src', 'antifantasma-cliente', 'antifantasma.cjs'), 'utf-8');
