/**
 * !vab18 — "Isso ou Aquilo" +18 (mesmo formato do `!vab`).
 *
 * O `!vab18` monta uma ENQUETE de duas opções, com a pergunta no TÍTULO — a
 * mesma mecânica do `!vab`; o que muda é só a lista (`vab18.json`, 200 itens).
 * Ele vive no `menu18`, na categoria BRINCADEIRAS.
 *
 * Este teste roda o handler REAL e confere os dois lados:
 *   - o arquivo (`vab18.json`): 200 itens, cada um com pergunta + 2 opções;
 *   - o comando: a enquete sai com a PERGUNTA no título e as duas opções.
 *
 * Uso: node tests/vab18.test.js
 */
...
const TMP_DB = fs.mkdtempSync(path.join(os.tmpdir(), 'lizzy-vab18-'));
...
const VAB18 = JSON.parse(fs.readFileSync(path.join(ROOT, 'dados/src/funcs/json/vab18.json'), 'utf-8'));
...
function makeGroup(extra = {}) {
  groupCounter += 1;
  const jid = `120363950000000${String(groupCounter).padStart(3, '0')}@g.us`;
  fs.writeFileSync(path.join(GRUPOS_DIR, `${jid}.json`),
    JSON.stringify({ groupName: `GV18 ${groupCounter}`, modobrincadeira: true, modo18: true, ...extra }, null, 2));
  return jid;
}
...
async function rodar({ text = '!vab18', groupData = {} } = {}) {
  const s = nextSender();
...
await test('vab18.json tem os 200 itens', () => {
  ok(Array.isArray(VAB18), 'é uma lista');
  ok(VAB18.length === 200, `200 itens (veio ${VAB18.length})`);
  ok(VAB18[0].pergunta === 'O que mais chama sua atenção em alguém?', 'primeira pergunta é a esperada');
  ok(VAB18[199].pergunta === 'Para terminar: o que seria mais perigoso?', 'última pergunta é a esperada');
});

await test('cada item tem pergunta e DUAS opções com texto', () => {
  const ruins = VAB18.filter((i) =>
  ...
  const iguais = VAB18.filter((i) => i.option1 === i.option2);
  ok(iguais.length === 0, 'opções diferentes entre si');
});

await test('nenhum ITEM repetido (pergunta + opções)', () => {
  const itens = VAB18.map((i) => `${i.pergunta}|${i.option1}|${i.option2}`);
  ok(new Set(itens).size === itens.length,
    `itens únicos (total ${itens.length}, únicos ${new Set(itens).size})`);
});

await test('os quatro blocos estão na ordem (atração / pegação / fantasia / ciúmes)', () => {
  ok(VAB18[0].pergunta.includes('chama sua atenção'), 'bloco 1 (atração) abre');
  ok(VAB18[50].pergunta.includes('tomar a iniciativa'), 'bloco 2 (pegação) começa no item 51');
  ok(VAB18[100].pergunta.includes('fantasia seria mais tentadora'), 'bloco 3 (fantasias) começa no item 101');
  ok(VAB18[150].pergunta.includes('ciúmes'), 'bloco 4 (ciúmes/tensão) começa no item 151');
});
...
await test('!vab18 publica enquete com a PERGUNTA no título e as duas opções', async () => {
...
await test('menu18 tem a categoria BRINCADEIRAS com o !vab18', async () => {
  ...
  ok(menu.includes('!vab18'), 'lista o !vab18');
  ok(menu.includes('PLAQUINHA'), 'mantém a categoria PLAQUINHA');
});

await test('a categoria BRINCADEIRAS vem depois de PLAQUINHA', async () => {
  const menu = desbold(await menus.menu18('!', 'Lizzy', 'Kannon'));
  ok(menu.indexOf('PLAQUINHA') < menu.indexOf('BRINCADEIRAS'), 'PLAQUINHA antes de BRINCADEIRAS');
});
